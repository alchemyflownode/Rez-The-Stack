@echo off
title REZ HIVE SOVEREIGN LAUNCHER v5.3 - CLEAN STRUCTURE
color 0A
...
if %errorlevel%==0 (
    ...
    if %errorlevel%==0 (
        ...
        echo   Please start Ollama manually from: %OLLAMA_PATH%
    )
)
echo.

:: Check if ChromaDB is installed in venv
if not exist "%VENV_CHROMA%" (
    echo [WARN] ChromaDB not found in venv - installing...
    "%VENV_PYTHON%" -m pip install chromadb
)
...
echo.
echo Logs are saved in: %PROJECT_PATH%\logs\
echo.
pause