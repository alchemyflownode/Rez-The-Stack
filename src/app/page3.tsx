...
import { 
  Brain, Eye, Hand, Database, Cpu, Zap, Network,
  Menu, ChevronDown, ChevronRight, HelpCircle, Mic, MicOff, Trash2, X
} from "lucide-react";
...
const TwoBarMeter = dynamic(
  () => import('../components/TwoBarMeter').then(m => m.TwoBarMeter),
  { ssr: false }
);
import remarkGfm from "remark-gfm";
import {
...
const METRICS_UPDATE_INTERVAL = 4000;
const SERVICES_CHECK_INTERVAL = 15000;
const DEBOUNCE_STORAGE_MS = 500;
const STREAM_MIN_UPDATE_MS = 50;
...
export default function SovereignDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedChat) {
        ...
        setChatLog(parsed.slice(-MAX_CHAT_HISTORY));
      } else {
        setChatLog([
  ...
  // --- Timestamp & Metrics Updater ---
  useEffect(() => {
    if (!isMountedRef.current) return;
    
    const timestampInterval = setInterval(() => {
      if (isMountedRef.current) setTimestamp(getTime());
    }, 1000);
  ...
  useEffect(() => {
    if (!isMountedRef.current || chatLog.length === 0) return;
    ...
    const handler = setTimeout(() => {
      if (isMountedRef.current && typeof window !== "undefined") {
        try {
          ...
          logger.warn("StorageSync", "Failed to sync chat to localStorage");
        }
      }
    }, DEBOUNCE_STORAGE_MS);
  ...
  const checkServices = useCallback(async () => {
    ...
    try {
      ...
      logger.warn("ServiceCheck", "Failed to check service status");
    }
  }, []);
  ...
  const fetchModels = useCallback(async () => {
    ...
    try {
      ...
      logger.warn("FetchModels", "Using fallback models");
      if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    const servicesInterval = setInterval(() => {
      if (isMountedRef.current) checkServices();
    }, SERVICES_CHECK_INTERVAL);
    return () => clearInterval(servicesInterval);
  }, [checkServices, fetchModels]);

  // --- Action Handler (Production-Tier) ---
  const executeAction = useCallback(async (commandText: string, payload?: Record<string, any>) => {
    if (!commandText.trim() || loading || !isMountedRef.current) return;
    ...
    setChatLog((prev) => {
      const updated: ChatMessage[] = [...prev, userMessage, { id: aiMessageId, role: "ai", content: "", timestamp: getTime() }];
    ...
    try {
      ...
      try {
        while (true) {
          ...
          // Throttle updates
          const now = Date.now();
          if (now - lastUpdate > STREAM_MIN_UPDATE_MS && isMountedRef.current) {
            setChatLog((prev) => 
              prev.map((msg) => 
      ...
      if (e.name === "AbortError") {
        logger.info("ExecuteAction", "Request cancelled");
      } else {
  ...
  }, [loading, activeWorker, selectedModel]);

  const startListening = useCallback(() => {
    // TODO: Implement Web Speech API integration
    setIsListening((prev) => !prev);
    if (!isListening) {
      logger.info("VoiceInput", "Voice input not yet implemented");
    }
  }, [isListening]);

  const clearChat = useCallback(() => {
    setChatLog([{ id: generateId(), role: "ai", content: WELCOME_MESSAGE, timestamp: getTime() }]);
    logger.info("UI", "Chat cleared");
  }, []);
      ...
      <header className="fixed top-0 left-0 right-0 h-12 bg-black/50 backdrop-blur-md border-b border-white/5 flex items-center px-4 z-30">
        ...
        <div className="flex items-center gap-3 ml-6">
          ...
          <div className="relative ml-2">
            ...
            {showModelDropdown && (
              <div className="absolute top-full mt-1 left-0 bg-black/90 border border-white/10 rounded-lg py-1 z-50 max-h-60 overflow-y-auto min-w-[160px]" role="listbox">
