...
# Run this from project root to verify all components
...
Write-Host "`n[6/6] Testing Configuration..." -ForegroundColor Cyan
try {
    $config = Get-Content "rez.config.json" | ConvertFrom-Json
    if ($config.identity.name -eq "Rez Hive Kernel") {
        Write-Status "Configuration valid - Phase: $($config.identity.phase)" "PASS" "Green"
        $results += @{Test="Configuration"; Status="PASS"; Detail="Phase $($config.identity.phase)"}
    } else {
        Write-Status "Configuration file exists but may be malformed" "WARN" "Yellow"
        $results += @{Test="Configuration"; Status="WARN"; Detail="Check format"}
    }
} catch {
    Write-Status "Configuration file missing or invalid" "WARN" "Yellow"
    $results += @{Test="Configuration"; Status="WARN"; Detail="Not found"}
}

# Test 7: Network Connectivity (if Deep scan requested)
if ($Deep) {
    Write-Host "`n[7/6] Deep Scan: Network Connectivity..." -ForegroundColor Cyan
    try {
